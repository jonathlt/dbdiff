...
CREATE OR REPLACE FUNCTION pg_temp.create_table_list()
  RETURNS TABLE(table_schema text, table_name text, rowcount bigint)
  LANGUAGE plpgsql AS
$func$
DECLARE
   formal_table text;
...
BEGIN
   ...
   sql_create_table := 'CREATE TEMPORARY TABLE table_list AS  
   						 SELECT table_schema::text, table_name::text, 0::bigint as rowcount 
   ...
   END LOOP;
   sql_result := 'SELECT table_schema, table_name, rowcount FROM table_list';
